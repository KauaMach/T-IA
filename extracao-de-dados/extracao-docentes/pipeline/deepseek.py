...
async def run_deepseek_pipeline(
    root_dir: str,
    ...
    async def process_file(filepath, relative_filepath, records):
        ...
        try:
            if ext in HTML_EXTENSIONS:
                ...
                image_bytes = image_to_png_bytes(filepath)
    ...
    for filepath in all_files:
        if is_compressed(filepath):
            ...
            records = await process_file(filepath, relative_filepath, records)
    ...
    logger.info(
        f"\nPipeline DeepSeek concluída! {len(all_files)} arquivos processados. "
